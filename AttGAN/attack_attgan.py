...
attgan.eval()
pgd_attack = attacks.LinfPGDAttack(model=attgan.G, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), feat=None)
for idx, (img_a, att_a) in enumerate(test_dataloader):
    ...
    for i, att_b in enumerate(att_b_list):
        ...
        # print(img_a.shape, gen_noattack.shape) # debug
        # Attacks
        x_adv, perturb = pgd_attack.universal_perturb(img_a, att_b_,gen_noattack, attgan)
    print(idx)
    if idx==100:
        break
...
for idx, (img_a, att_a) in enumerate(test_dataloader):
    ...
    for i, att_b in enumerate(att_b_list):
        ...
        if i > 0:
            att_b_[..., i - 1] = att_b_[..., i - 1] * args.test_int / args.thres_int
        with torch.no_grad():
            gen_noattack = attgan.G(img_a+pgd_attack.up, att_b_)
        samples.append(gen_noattack)
